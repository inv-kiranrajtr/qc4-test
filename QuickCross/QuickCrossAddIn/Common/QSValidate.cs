...
namespace ExcelAddIn.Common
{
	class QSValidate
	{
		...
		internal QSValidate(Workbook workbook)
		{
			...
			questionSettingSheet = ExcelUtil.GetWorkSheetByCodeName(workbook, Constants.SheetCodeName.QuestionSetting);
		...
		public ReturnClass QuestionConfigurationCheck()
		{
			...
			{
				return new ReturnClass(false, GetRange(2, 12), AddinResource.QS_ALERT_QTITLE_NOT_EMPTY);
			}

			Range qsRange = questionSettingSheet.Range[Constants.QS.QsStartAddress];
			qsRange = ExcelUtil.EndxlUp(qsRange, true);
			int lastRow = 4;
		...
		private ReturnClass ValidateVariable(object obj, out string variable, ref List<string> varList)
		{
			...
			{
				return new ReturnClass(false, GetRange(rowNum, Constants.QS.QsColItem), AddinResource.QS_VARIABLE_NULL_NOT_ALLOWED, new string[] { rowNum.ToString() });
			}

			string item = variable;
			if (varList.Any(s => s.Equals(item, StringComparison.OrdinalIgnoreCase))) // TODO case check
			{
				return new ReturnClass(false, GetRange(rowNum, Constants.QS.QsColItem), AddinResource.QS_VARIABLE_DUPLICATED, new string[] { rowNum.ToString() });
			}
			string val = String.Empty;
			if (null != obj)
			{
		...
		private ReturnClass ValidateQuestionType(Object obj, out string qType)
		{
			...
			{
				return new ReturnClass(false, GetRange(rowNum, Constants.QS.QsColQuestiontype), AddinResource.QS_ALERT_INVALID_QUESTION_TYPE, new string[] { rowNum.ToString() });
			}
			return new ReturnClass(true);
		}
		...
		private ReturnClass ValidateQuestionCount(object obj, out int questionCount, string questionType, int categoryCount, string variable, int noOfSubTotal, string answerType)
		{
			...
			{
				return new ReturnClass(false, GetRange(rowNum, Constants.QS.QsColNumberOfQuestion), String.Format(AddinResource.SET_INETGER_BETWEEN, 1, 200), new string[] { rowNum.ToString(), questionType });
			}
			...
			{
				return count == 1 ? new ReturnClass(true) : new ReturnClass(false, GetRange(rowNum, Constants.QS.QsColNumberOfQuestion), AddinResource.QS_QUESTION_COUNT_SHOULD_EMPTY, new string[] { rowNum.ToString(), questionType });
			}
			if (!Int32.TryParse(qNumber, out questionCount))
			{
				return new ReturnClass(false, GetRange(rowNum, Constants.QS.QsColNumberOfQuestion), AddinResource.QS_QUESTION_COUNT_INTEGER, new String[] { rowNum.ToString() });
			}
			if (questionCount < 0)
			{
				return new ReturnClass(false, GetRange(rowNum, Constants.QS.QsColNumberOfQuestion), AddinResource.QS_QUESTION_COUNT_NEGATIVE, new String[] { rowNum.ToString() });
			}
				...
				|| Constants.QuestionType.MTM.Equals(questionType))
			{
				...
				{
					return new ReturnClass(false, GetRange(rowNum, Constants.QS.QsColNumberOfQuestion), AddinResource.QS_QUESTION_COUNT_MORETHAN_201, new string[] { rowNum.ToString(), questionType });
				}
			}
			...
			for (int j = 1; j < questionCount; j++)
			{
				try
				{
					...
					{
						return new ReturnClass(false, GetRange(rowNum + j, Constants.QS.QsColCategories), AddinResource.QS_SET_INTEGER, new string[] { "1", "1000", (rowNum + j).ToString(), (Constants.QS.QsColCategories).ToString() });
					}
					...
					{
						return new ReturnClass(false, GetRange(rowNum + j, Constants.QS.QsColQuestionNumber), AddinResource.QS_MATRIX_QUESTION_LESS, new string[] { variable, (rowNum + j).ToString() });
					}
					...
					{
						return new ReturnClass(false, GetRange(rowNum + j, Constants.QS.QsColAnswerType), AddinResource.QS_INVALID_ANSWER_TYPE, new string[] { (rowNum + j).ToString(), variable });
					}

					if (c != categoryCount)
					{
						return new ReturnClass(false, GetRange(rowNum + j, Constants.QS.QsColCategories), AddinResource.QS_MATRIX_CHOICE_COUNT_NOT_EQUAL, new string[] { (rowNum + j).ToString() });
					}
					...
					{
						return new ReturnClass(false, GetRange(rowNum + j, Constants.QS.QsColWT), AddinResource.QS_MATRIX_WT_NOT_EQUAL, new string[] { (rowNum + j).ToString(), (Constants.QS.QsColWT).ToString() });
					}

					if (Convert.ToString(objAry[i + j, Constants.QS.QsColSortDisplay]) != Convert.ToString(objAry[i + j - 1, Constants.QS.QsColSortDisplay]))
					{
						return new ReturnClass(false, GetRange(rowNum + j, Constants.QS.QsColSortDisplay), AddinResource.QS_MATRIX_SORT_NOT_EQUAL, new string[] { (rowNum + j).ToString(), (Constants.QS.QsColSortDisplay).ToString() });
					}
					...
					for (int k = 0; k < noOfSubTotal; k++)
					{
						...
						if (!str1.Equals(str2))
						{
							...
							{
								message = AddinResource.QS_ERROR_IN_MATRIX_SUBTOTAL_COUNT;
							}
							else if (startCol + k == Constants.QS.QsColCountBase)
							{
								message = AddinResource.QS_ERROR_IN_MATRIX_SUBTOTAL_COUNTBASE;
							}
							else if (startCol + k == Constants.QS.QsColAddSunTotal)
							{
								message = AddinResource.QS_ERROR_IN_MATRIX_SUBTOTAL_ADDSUB;
							}
							else if (startCol + k == Constants.QS.QsColNumberSubTotal)
							{
								message = AddinResource.QS_ERROR_IN_MATRIX_SUBTOTAL_ADDSUBCOUNT;
							}
							else
							{
								message = AddinResource.QS_ERROR_IN_MATRIX_SUBTOTAL;
							}
				...
				catch (Exception ex)
				{
					...
					return new ReturnClass(false, GetRange(rowNum, Constants.QS.QsColNumberOfQuestion), AddinResource.QS_ERROR_IN_MATRIX_QUESTION, new string[] { rowNum.ToString(), Constants.QS.QsColNumberOfQuestion.ToString() });
				}
			}
			return new ReturnClass(true);
		}
		...
		private ReturnClass ValidateAnswerType(Object obj, out string answerType, string questionType, string variable, int row)
		{
			...
			{
				return new ReturnClass(false, GetRange(rowNum, Constants.QS.QsColAnswerType), AddinResource.QS_NULL_ANSWER_TYPE, new string[] { rowNum.ToString(), variable });
			}
			...
			switch (answerType)
			{
				...
				default:
					...
					{
						return new ReturnClass(false, GetRange(row, Constants.QS.QsColAnswerType), AddinResource.QS_INVALID_ANSWER_TYPE, new string[] { rowNum.ToString(), variable });
					}
					else
					{
						return new ReturnClass(false, GetRange(row, Constants.QS.QsColAnswerType), AddinResource.QS_RAWDATA_ANSWER_TYPE_NOT_MATCH, new string[] { rowNum.ToString(), variable, answerType, Definitions.VariableDictionary[variable].AnswerTypeBefore });
					}
					break;
			}
		...
		private ReturnClass ValidateCategoryCount(Object obj, out int categoryCount, string answerType, string variable)
		{
			...
			else
			{
					...
					)
				{
					...
					{
						return new ReturnClass(false, GetRange(rowNum, Constants.QS.QsColCategories), AddinResource.QS_SET_INTEGER, new string[] { "1", "1000", rowNum.ToString(), Constants.QS.QsColCategories.ToString() });
					}
					return CheckUnwantedChoice(categoryCount);
				}
				else
				{
					return new ReturnClass(false, GetRange(rowNum, Constants.QS.QsColCategories), AddinResource.QS_NUMBER_OF_CHOICE_SET, new string[] { rowNum.ToString(), answerType, variable });
				}
			}
			return new ReturnClass(false, GetRange(rowNum, Constants.QS.QsColCategories), AddinResource.QS_NUMBER_OF_CHOICE_NOT_SET, new string[] { rowNum.ToString(), variable });
		}
		...
		private ReturnClass CheckUnwantedChoice(int categoryCount)
		{
			...
			{
				return new ReturnClass(false, left, AddinResource.QS_CHOICE_VALUE_MORETHAN_COUNT, new string[] { left.Row.ToString(), left.Column.ToString() });
			}
			return new ReturnClass(true);
		}
		...
		private ReturnClass ValidateWT(Object obj, int categoryCount, string variable)
		{
			...
			{
				return new ReturnClass(false, GetRange(rowNum, Constants.QS.QsColWT), AddinResource.QS_ALERT_WT, new string[] { rowNum.ToString(), variable });
			}
		...
		private ReturnClass ValidateSortTab(Object obj, int categoryCount)
		{
			...
			if (categoryCount == 0)
			{
				return new ReturnClass(false, GetRange(rowNum, Constants.QS.QsColSortDisplay), AddinResource.QS_ALERT_INVALID_VALUE, new string[] { rowNum.ToString(), Constants.QS.QsColSortDisplay.ToString() });
			}

			if (!Int32.TryParse(str, out sortVal))
			{
				return new ReturnClass(false, GetRange(rowNum, Constants.QS.QsColSortDisplay), AddinResource.SET_NUMERIC_VALUE_RANGE, new string[] { rowNum.ToString(), Constants.QS.QsColSortDisplay.ToString() });
			}
			if (sortVal != 0 && sortVal <= categoryCount)
			{
				return new ReturnClass(true);
			}
			return new ReturnClass(false, GetRange(rowNum, Constants.QS.QsColSortDisplay), AddinResource.QS_ALERT_INVALID_VALUE_NOT_BETWEEN, new string[] { categoryCount.ToString() });
		}
		...
		private ReturnClass ValidateCount(Object obj, int catCount, string answerType, int column, out String count)
		{
			...
			{
				return new ReturnClass(false, GetRange(rowNum, column), AddinResource.QS_NO_VALUE_REQUIRED, new string[] { rowNum.ToString(), column.ToString() });
			}

			if (!QC4Common.Validation.NumberCheck.NUmberCheckSubtotal(count, catCount))
			{
				rc.Result = false;
				rc.Msg = String.Format(AddinResource.SET_INETGER_BETWEEN, 1, catCount);
		...
		private ReturnClass ValidateCountBase(Object obj, string answertype, int column, string countValue)
		{
			...
			{
				return new ReturnClass(false, GetRange(rowNum, column), AddinResource.QS_ALERT_INVALID_COUNTBASE_FLAG, new string[] { rowNum.ToString() });
			}
			return new ReturnClass(true);
		}
		...
		private ReturnClass ValidateSubTotal(Object obj, out int subTotal, string answerType, int maxValue, int column, int addSubTotal)
		{
			...
			{
				return new ReturnClass(false, GetRange(rowNum, column), AddinResource.QS_ALERT_ADD_SUBTOTAL_COUNT_NOT_SET, new string[] { rowNum.ToString(), column.ToString() });
			}
			...
			return new ReturnClass(false, GetRange(rowNum, column), AddinResource.QS_ALERT_INVALID_VALUE, new string[] { rowNum.ToString(), column.ToString() });
		}
		...
		private ReturnClass ValidateSubtotalAndCondition(int subTotalCount, int catCount)
		{
			...
			{
				return new ReturnClass(false, range, AddinResource.QS_NO_VALUE_REQUIRED, new string[] { range.Row.ToString(), range.Column.ToString() });
			}
			...
			for (int j = 0; j < subTotalCount; j++)
			{
				...
				{
					return new ReturnClass(false, GetRange(rowNum, Constants.QS.QsColSubtotal1 + j * 2), AddinResource.QS_ALERT_ADD_SUBTOTAL_NAME_NOT_SET, new string[] { rowNum.ToString(), (Constants.QS.QsColSubtotal1 + j * 2).ToString() });
				}

				if (IsNullOrEmpty(objAry[i, Constants.QS.QsColcriteria1 + j * 2], out str))
				{
					return new ReturnClass(false, GetRange(rowNum, Constants.QS.QsColcriteria1 + j * 2), AddinResource.QS_ALERT_ADD_SUBTOTAL_CRITERIA_NOT_SET, new string[] { rowNum.ToString(), (Constants.QS.QsColSubtotal1 + j * 2).ToString() });
				}
