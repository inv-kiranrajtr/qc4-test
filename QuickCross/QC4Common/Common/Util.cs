...
namespace QC4Common.Common
{
    ...
    public class Util
    {
        ...
        public static int IsVariableFoundInQSSheet(Excel.Workbook book ,string TargetCellContent)
        {

            int FoundAtRow = -1;
            Excel.Worksheet wsQstnSettings = ExcelUtilForAddIn.GetWorkSheetByCodeName(book,Constants.SheetCodeName.QuestionSetting);//ExcelUtil.GetWorksheetByIndex(1);
            Excel.Range range = ExcelUtilForAddIn.GetNamedRange(book,"List", "List_Item_ALLD");
		...
		public static int IsVariableFoundInList(Excel.Workbook book,string TargetCellContent,string ListType = Constants.VariableList.ListItemALLD)
		{

			int FoundAtRow = -1;
			Excel.Worksheet wsQstnSettings = ExcelUtilForAddIn.GetWorksheetByCodeName(book,Constants.SheetType.sh_ListView);
			Excel.Range range = ExcelUtilForAddIn.GetNamedRange(book,"List", ListType);
        ...
        public static DataTable LoadDataFromDB(Excel.Workbook book,string sql)
        {
           
            DataTable dt = new DataTable();
            try
            {
                using (SQLiteConnection dbSource = DBHelper.GetConnection(DBHelper.GetConnectionString(book)))//DataProcess.Sheet.Application.ActiveWorkbook
                {
        ...
        public static int GetTotalRowcount(string connectionstring = "")
        {
            ...
            try
            {
                using (SQLiteConnection dbSource = DBHelper.GetConnection(!string.IsNullOrEmpty(connectionstring) ? connectionstring : DBHelper.GetConnectionString(DataProcess.currworkbook)))//DataProcess.Sheet.Application.ActiveWorkbook
                {
        ...
        public static int GetTotalRowCountForDeleteLDEL(int sortnumbr = 0, int deletedrows = 0)
        {
            ...
            try
            {
                ...
                using (SQLiteConnection dbSource = DBHelper.GetConnection(DBHelper.GetConnectionString(DataProcess.currworkbook)))//DataProcess.Sheet.Application.ActiveWorkbook
                {
        ...
        public static int GetWIndowHandleFromSettings(Excel.Workbook book)
        {
            IntPtr hWnd_Processhandle;
            int ProcesId = Convert.ToInt32(QC4Common.Util.QCUtil.GetProcessId(book.Application.ActiveWorkbook));
        ...
        public static IntPtr GetIntPtrWIndowHandleFromSettings(Excel.Workbook book)
        {
            IntPtr hWnd_Processhandle;
            int ProcesId = Convert.ToInt32(QC4Common.Util.QCUtil.GetProcessId(book.Application.ActiveWorkbook));
            Process hostProcess = Process.GetProcessById(ProcesId);
        ...
        public static bool checkUnprocessedNewQuestionDialog(Excel.Workbook workBook, IntPtr active)
        {
            ...
            {
                System.Windows.Forms.DialogResult dialogResult = MessageDialog.InfoYesNo(CommonResource.ALERT_UN_PROCESSED_VARIABLE_EXIST, active);
                if (dialogResult == System.Windows.Forms.DialogResult.No)
                {
                    MessageDialog.Info(CommonResource.ALERT_EXECUTE_DP_OR_DELET_VAR, active);
                    return true;
                }
            }
            return false;
