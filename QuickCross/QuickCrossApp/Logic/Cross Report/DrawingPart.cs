...
namespace Qc4Launcher.Logic.Cross_Report
{
    public class DrawingPart
    {
        ...
        // Top GWS combo chart: clustered columns + line series on the same category/value axes.
                                             ...
                                             ChartPart chartPart, int firstRow, int lastRow, int firstCol, int lastCol, bool isN, string tempTable, int i, int[] MaxAxesCountArray)
        {
            ...
            if (HasLines)
            {
                string barLabel = GetChartCellText(worksheetPart, categoryRow, firstCol);
                if (string.IsNullOrEmpty(barLabel))
                    barLabel = "全体";
                barChartSeries1.Append(CreateCachedSeriesText(tempTable, categoryRow, firstCol, barLabel));
            }
            ...
            if (HasLines)
            {
                ...
                for (int j = 0; j < LinesIndexList.Count; j++)
                {
                    if (Convert.ToInt32(LinesIndexList[j]) >= 2 || Convert.ToInt32(LinesIndexList[j]) <= v.GetUpperBound(0) - (1 + 1))
                    {
                        ...
                        string seriesLabel = OutputUtil.RemoveLeadingSpclChar(String.Join(" - ", tmpBuf));
                        if (string.IsNullOrEmpty(seriesLabel))
                            seriesLabel = "系列" + (j + 1);

                        lineChartSeries1.Append(index2);
                        lineChartSeries1.Append(order2);
                        lineChartSeries1.Append(CreateCachedSeriesText(tempTable, fRow, firstCol, seriesLabel));
                        ...
                        C.Smooth smooth1 = new C.Smooth() { Val = false };
                        // Thicker stroke (sample GWS combo uses ~25000 EMUs) so lines survive Sheets import.
                        lineChartSeries1.Append(SetLineChartProperty(rgb, 25000));
                        lineChartSeries1.Append(marker1);
                ...
                C.DataLabels lineDataLabels = new C.DataLabels();
                lineDataLabels.Append(new C.ShowLegendKey() { Val = false });
                lineDataLabels.Append(new C.ShowValue() { Val = false });
                lineDataLabels.Append(new C.ShowCategoryName() { Val = false });
                lineDataLabels.Append(new C.ShowSeriesName() { Val = false });
                lineDataLabels.Append(new C.ShowPercent() { Val = false });
                lineDataLabels.Append(new C.ShowBubbleSize() { Val = false });
                lineChart1.Append(lineDataLabels);
                lineChart1.Append(new C.ShowMarker() { Val = true });
                lineChart1.Append(new C.Smooth() { Val = false });
                lineChart1.Append(new C.AxisId() { Val = (UInt32Value)2145344767U });
                lineChart1.Append(new C.AxisId() { Val = (UInt32Value)2145223903U });
            }
            ...
            // Connect through blanks/zeros so GWS line series remain visible across categories.
        ...
        public static C.ChartShapeProperties SetLineChartProperty(string colur, int width = 3175)
        {
            C.ChartShapeProperties chartShapeProperties3 = new C.ChartShapeProperties();

            A.Outline outline6 = new A.Outline() { Width = width };
        ...
        public static C.CategoryAxisData SetStringDataLinkValues(WorksheetPart worksheetPart, string perSheetName, int perStartRow, int endRow, int perLastCol, int perFirstCol = 0)
        {
            ...
            stringReference1.Append(formula1);
            // Google Sheets requires strCache on chart refs to import combo (line) series.
        ...
        public static C.XValues SetNumericXDataLinkValues(WorksheetPart worksheetPart, string sheetName, int firstRow, int lastRow, int lastCol, int firstCol = 0, UInt32 numberOfPoints = 0)
        {
            ...
            else if (numberOfPoints > 0)
            {
                ...
                numberingCache3.Append(new C.FormatCode() { Text = "0.0" });
                numberingCache3.Append(new C.PointCount() { Val = (UInt32Value)numberOfPoints });
        ...
        private static C.NumberingCache BuildNumberingCacheIfPresent(WorksheetPart worksheetPart, int startRow, int endRow, int startCol, int endCol, string formatCodeText)
        {
            ...
            numberingCache.Append(new C.FormatCode() { Text = formatCodeText });
