...
namespace Qc4Launcher.Logic.Cross_Report
{
    public class DrawingPart
    {
        ...
        // Top GWS combo chart: vertical hidden data + sample-compatible barChart/lineChart axes.
        public static void GenerateColumClusterAndLineGraph(WorksheetPart worksheetPart, CrossTable tmpTable, string sheetName, string lineColour, List<int> LinesIndexList, ref Array v, bool HasLines,
                                             ChartPart chartPart, int firstRow, int lastRow, int firstCol, int lastCol, bool isN, string tempTable, int i, int[] MaxAxesCountArray)
        {
            GwsComboChartBuilder.BuildLandscapeColumnLineChart(
                worksheetPart, tmpTable, chartPart, LinesIndexList, ref v, HasLines,
                firstRow, firstCol, lastCol, i, MaxAxesCountArray, tempTable);
            return;
        }

        public static string GetChartCellTextPublic(WorksheetPart worksheetPart, int rowIdx, int colIdx)
        {
            return GetChartCellText(worksheetPart, rowIdx, colIdx);
        }
