...
namespace Qc4Launcher.Logic.Cross_Report
{
    public class DrawingPart
    {
                                                 ...
                                                 ChartPart chartPart, int firstRow, int lastRow, int firstCol, int lastCol, bool isN, string tempTable, bool HasLines, List<int> LinesIndexList)
        {
            ...
            // Scatter with LineMarker on secondary axes — Excel/GWS visual: horizontal bars + peaked lines.
            // Do not replace with lineChart on shared axes (that flips to vertical columns + floating markers).
            ...
            if (HasLines)
            {
                ...
                for (int j = 0; j < LinesIndexList.Count; j++)
                {
                    ...
                    scatterChartSeries1.Append(SetSeriesText(tempTable, sideTableChoiceRow, sideTableFirstCol, seriesLabel));
                    ...
                    // x = series % values, y = category slots (0.5, 1.5, ...) so lines peak across horizontal bars
                    ...
                    UInt32Value index = 0;
                    for (int r = firstRow; r <= lastRow; r++, pointValue += 1)
                    {
